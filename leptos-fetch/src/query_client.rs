...
use leptos::{
    prelude::{
        ArcRwSignal, ArcSignal, Effect, Get, GetUntracked, LocalStorage, Owner, Read, ReadUntracked, Set, Signal,
        Track, provide_context,
    },
    server::{ArcLocalResource, ArcResource, FromEncodedStr, IntoEncodedString, LocalResource, Resource},
};
use send_wrapper::SendWrapper;

use crate::{
    ArcLocalSignal, QueryOptions,
    cache::{CachedOrFetchCbInput, CachedOrFetchCbInputVariant, CachedOrFetchCbOutput, OnScopeMissing},
...
impl<Codec: 'static> QueryClient<Codec> {
    ...
    ///
    /// The current `codee` major version is `0.3` and will need to be
    /// imported in your project to customize the codec.
    ///
    ...
    /// ```
    ///
    /// [`codee::binary::MsgpackSerdeCodec`]: https://docs.rs/codee/latest/codee/binary/struct.MsgpackSerdeCodec.html
    ///
    /// This is a generic type on the [`QueryClient`], so when calling
    ...
    pub fn with_refetch_enabled_toggle(self, refetch_enabled: impl Into<ArcSignal<bool>>) -> Self {
        self.untyped_client.scope_lookup.scopes_mut().refetch_enabled = Some(refetch_enabled.into());
        self
    }
    ...
    pub fn refetch_enabled(&self) -> Option<ArcSignal<bool>> {
        self.untyped_client.scope_lookup.scopes().refetch_enabled.clone()
    }

    /// Query with [`LocalResource`]. Local resouces only load data on the client,
    /// so can be used with non-threadsafe/serializable data.
    ///
    /// If a cached value exists but is stale, the cached value will be initially used,
    /// then refreshed in the background, updating once the new value is ready.
    #[track_caller]
    ...
    /// Query with [`ArcLocalResource`]. Local resouces only load data on the client,
    /// so can be used with non-threadsafe/serializable data.
    ///
    /// If a cached value exists but is stale, the cached value will be initially used,
    /// then refreshed in the background, updating once the new value is ready.
        ...
        V: DebugIfDevtoolsEnabled + Clone + 'static,
    {
        ...
        let drop_guard = ResourceDropGuard::<K, V>::new(self.untyped_client.scope_lookup, resource_id, cache_key);
        ...
        ArcLocalResource::new({
            move || {
                ...
                // it prevents the resource itself dropping when
                // the owner is held by the resource itself, here
                // each instance only lasts as long as the query.
                let owner_chain = OwnerChain::new(client.untyped_client, query_scope_info.cache_key, Owner::current());
                async move {
                            ...
                            .cached_or_fetch(
                                ...
                                &key,
                                {
                                    ...
                                    move |key| {
                                        ...
                                        async move { MaybeLocal::new_local(query_scope.query(key).await) }
                                    }
                                },
                                |info| {
                                    ...
                                    match info.variant {
                                        CachedOrFetchCbInputVariant::CachedUntouched => {
                                                ...
                                                && info.cached.stale_or_invalidated()
                                            {
                                                ...
                                                // Just adding the SendWrapper and using
                                                // spawn() rather than spawn_local()
                                                // to fix tests:
                                    ...
                                    CachedOrFetchCbOutput::Return(
                                        // WONTPANIC: cached_or_fetch will
                                        // only output values that are safe
                                        // on this thread:
    ...
    {
        self.arc_resource_with_options(query_scope, keyer, false).into()
    }
    ...
    where
        ...
        <Codec as Decoder<MaybeKey::MappedValue>>::Encoded: FromEncodedStr,
    {
        self.arc_resource_with_options(query_scope, keyer, true).into()
    }
        ...
        <Codec as Decoder<MaybeKey::MappedValue>>::Encoded: FromEncodedStr,
    {
        ...
        let resource = ArcResource::new_with_options(
            {
                ...
                move || {
                    if let Some(key) = keyer().into_maybe_key() {
                        ...
                        scope_lookup.with_cached_query::<K, V, _>(&key_hash, &cache_key, |maybe_cached| {
            ...
            },
            {
                ...
                move |(maybe_key, last_used_buster)| {
                    ...
                    let query_scope_info = query_scope_info.clone();
                    let buster_if_uncached = buster_if_uncached.clone();
                    // Want the guard around everywhere
                    // until the resource is dropped.
                    let _drop_guard = drop_guard.clone();
                    // Note: cannot hoist outside of resource,
                    // it prevents the resource itself dropping
                    // when the owner is held by the resource
                    // itself, here each instance only lasts
                    // as long as the query.
                    let owner_chain =
                        OwnerChain::new(client.untyped_client, query_scope_info.cache_key, Owner::current());
                    async move {
                        if let Some(key) = maybe_key {
                            ...
                            let value = client
                                ...
                                .cached_or_fetch(
                                    ...
                                    &key,
                                    {
                                        ...
                                        move |key| {
                                            let query_scope = query_scope.clone();
                                            async move { MaybeLocal::new(query_scope.query(key).await) }
                                        }
                                    },
                                    |info| {
                                        info.cached.mark_resource_active(resource_id);
                                        match info.variant {
                                            CachedOrFetchCbInputVariant::CachedUntouched => {
                                                ...
                                                // Handle edge case where the key
                                                // function saw it was uncached, so
                                                // entered here, but when
                                                // cached_or_fetch was acquiring the
                                                // fetch mutex, someone else cached
                                                // it, meaning our key function won't
                                                // be reactive correctly unless we
                                                // invalidate it now:
                                                if last_used_buster != info.cached.buster.get_untracked() {
                                                    buster_if_uncached.set(info.cached.buster.get_untracked());
                                                }
                                            }
                                            CachedOrFetchCbInputVariant::Fresh => {}
                                            CachedOrFetchCbInputVariant::CachedUpdated => {
        ...
        // On the client, want to repopulate the frontend cache,
        // so should write resources to the cache here if they
        // don't exist.
        // It would be better if in here we could check if the
        // resource was started on the backend/streamed, saves
        // doing most of this if already a frontend resource.
        let effect = {
            let resource = resource.clone();
            let buster_if_uncached = buster_if_uncached.clone();
            let client_created_at = self.untyped_client.created_at;
            // Converting to Arc because the tests like the client
            // get dropped even though this persists:
                        ...
                        scope_lookup.with_cached_scope_mut::<K, V, _, _>(
                            ...
                            |maybe_scope, _| {
                                ...
                                if let Some(key) = keyer().into_maybe_key() {
                                    ...
                                    // calling client.resource() multiple
                                    // times, previous impl would only run
                                    // the effect once per
                                    // client.resource(), but would still
                                    // run again on subsequent
                                    // client.resource() with the same key
                                    // value
                                    // - client.resource()
                                    // - invalidate, starts loading in bg
                                    // - client.resource()
                                    // - effect runs again, sees resource
                                    //   has value, goes to set it, fires
                                    //   SsrStreamedValueOverride which
                                    //   cancels the actual new query
                                    //   loading to replace invalidated one
                                    // So have to have a client wide
                                    // protector, to make sure the effect
                                    // really only runs once on the client,
                                    // even if resource() called multiple
                                    // times.
                                        ...
                                        (Utc::now() - client_created_at).num_seconds() < 10
                                    {
                                        ...
                                        type AlreadySeenMap = HashMap<(u64, ScopeCacheKey, KeyHash), ()>;
                                        static ALREADY_SEEN: LazyLock<Mutex<AlreadySeenMap>> =
                                    ...
                                    // cancel any frontend query that
                                    // started already if there was a
                                    // client side local resource or
                                    // prefetch/fetch_query etc that
                                    // started on initial client-side ticks:
                                    if let Some(QueryOrPending::Pending { query_abort_tx, .. }) =
                                        scope.get_mut_include_pending(&key_hash)
                                    ...
                                    if was_pending || !scope.contains_key(&key_hash) {
                                        let query = Query::new(
                                            ...
                                            key_hash,
                                            MaybeLocal::new(key),
                                            MaybeLocal::new(MaybeKey::mapped_to_maybe_value(val.clone()).expect(
                                                "Just checked \
                                                    MaybeKey::mapped_value_is_some() \
                                                    is true",
                                            )),
                                            ...
                                            crate::events::Event::new(crate::events::EventVariant::StreamedFromServer),
                                        );
    ...
    {
        let query_scope_info = QueryScopeInfo::new(&query_scope);
        let owner_chain = OwnerChain::new(self.untyped_client, query_scope_info.cache_key, Owner::current());
        self.prefetch_inner(
            query_scope_info,
    ...
    where
        ...
        V: DebugIfDevtoolsEnabled + Clone + 'static,
    {
        self.untyped_client.fetch_query_local(query_scope, key).await
    }

    /// Set the value of a query in the cache.
    /// This cached value will be available from all threads and take priority
    /// over any locally cached value for this query.
    ///
    ...
    /// This cached value will only be available from this thread, the cache will
    /// return empty, unless a nonlocal value is set, from any other thread.
    ///
    /// Active resources using the query will be updated.
    #[track_caller]
        ...
        V: DebugIfDevtoolsEnabled + Clone + 'static,
    {
        ...
        self.untyped_client.scope_lookup.with_cached_scope_mut::<K, V, _, _>(
    ...
    ///
    /// If you want async and/or always get the value, use
    /// [`QueryClient::update_query_async`]/[`QueryClient::update_query_async_local`].
    ///
        ...
        V: DebugIfDevtoolsEnabled + Clone + 'static,
    {
        ...
        let maybe_return_value = self.untyped_client.scope_lookup.with_cached_scope_mut::<K, V, _, _>(
            &mut self.untyped_client.scope_lookup.scopes_mut(),
            query_scope_info.cache_key,
            OnScopeMissing::Skip,
            |_| {},
        ...
        FetcherFut: Future<Output = MaybeLocal<V>>,
    {
        ...
        // By holding the fetcher mutex from start to finish, prevent the chance of the value
        // being fetched between the fetch async call and the external user mapper async call
        // and the final set().
        ...
        self.untyped_client
            ...
            .with_notify_fetching(query_scope_info.cache_key, key_hash, false, async {
                ...
                let query_abort_rx = self.untyped_client.scope_lookup.prepare_invalidation_channel::<K, V>(
                    &query_scope_info,
                    key_hash,
                    &lazy_maybe_local_key(),
                );

                let result_fut = ASYNC_TRACK_UPDATE_MARKER.scope(track.clone(), async { mapper(&mut new_value).await });

                // Call the user async function, but also monitor for invalidations:
                // Specifically, we care about if .clear() is called.
                // If so, we shouldn't set the value, as it will have been generated with
                // the cleared cached query value.
                // If invalidated should still set, because the value will be deemed
                // "less stale", but won't de-invalidate it.
                ...
                if !cleared_during_user_fn {
                    ...
                    // - replacement value .set() called, stored in local cache because
                    //   no type safety on threadsafe cache
                    ...
                    if !updated {
                        self.set_inner::<K, V>(
                            ...
                            // fetch_inner would've reset it if needed, then the update
                            // itself shouldn't change the value:
                            ResetInvalidated::NoReset,
                        );
                    }
                }
    ...
    where
        ...
        V: DebugIfDevtoolsEnabled + Clone + 'static,
    {
        self.untyped_client.scope_lookup.with_cached_query::<K, V, _>(
    ...
    /// This is in contrast to `is_fetching`, that is `true` whenever the query
    /// is fetching data, including when it's refetching.
    ///
    /// From a resource perspective:
    /// - `is_loading=true`, the resource will be in a pending state until ready and implies `is_fetching=true`
    /// - `is_fetching=true` + `is_loading=false` means the resource is showing
    ///   previous data, and will update once new data finishes refetching
    ...
    /// This is in contrast to `is_loading`, that is `true` when the query is in
    /// the process of fetching data for the first time only.
    ///
    /// From a resource perspective:
    /// - `is_loading=true`, the resource will be in a pending state until ready and implies `is_fetching=true`
    /// - `is_fetching=true` + `is_loading=false` means the resource is showing
    ...
    {
        let cache_key = query_scope.cache_key();
        let keyer = ArcLocalSignal::derive_local(move || keyer().into_maybe_key().map(|k| KeyHash::new(&k)));
        let dyn_signal = self
            .untyped_client
            .scope_lookup
            .scope_subscriptions_mut()
        ...
        V: DebugIfDevtoolsEnabled + Clone + 'static,
    {
        ...
        drop(scopes);
        run_external_callbacks(self.untyped_client, query_scope_info.cache_key, cbs_external);
    }
    ...
    ///
    /// [`QueryClient::invalidate_all_queries`] on the other hand, will only refetch
    /// active queries in the background, replacing them when ready.
    #[track_caller]
    pub fn clear(&self) {
        let mut scopes = self.untyped_client.scope_lookup.scopes_mut();
        let mut cbs_scopes = vec![];
    ...
    #[cfg(test)]
    #[allow(dead_code)]
    pub(crate) fn mark_key_valid<K, V, M>(&self, query_scope: impl QueryScopeLocalTrait<K, V, M>, key: impl Borrow<K>)
    where
        K: DebugIfDevtoolsEnabled + Hash + Clone + 'static,
        V: DebugIfDevtoolsEnabled + Clone + 'static,
    {
    ...
    pub(crate) fn subscriber_count(&self) -> usize {
        self.untyped_client.scope_lookup.scope_subscriptions_mut().count()
    }
}
...
impl UntypedQueryClient {
        ...
        KRef: Borrow<K>,
    {
        ...
        let key_hashes = keys.iter().map(|key| KeyHash::new(key.borrow())).collect::<Vec<_>>();
        ...
        FetcherFut: Future<Output = MaybeLocal<V>>,
    {
        ...
        let next_directive =
            scope_lookup.with_cached_query::<K, V, _>(&key_hash, &query_scope_info.cache_key, |maybe_cached| {
                if let Some(cached) = maybe_cached {
        ...
        match next_directive {
            ...
            CachedOrFetchCbOutput::Refetch => {
                ...
                let _maybe_fetcher_mutex_guard_local = if maybe_preheld_fetcher_mutex_guard.is_none() {
                ...
                #[cfg(any(all(debug_assertions, feature = "devtools"), feature = "devtools-always"))]
                {
                    ...
                    if loading_first_time {
                        scope_lookup.client_subscriptions_mut().notify_query_created(
                            crate::subs_client::QueryCreatedInfo {
                                ...
                                combined_options: crate::options_combine(client_options, scope_options),
                            },
                        );
                    }
                }
                ...
                let maybe_new_value = scope_lookup
                    .with_notify_fetching(
                        ...
                        async {
                            loop {
                                let query_abort_rx = scope_lookup.prepare_invalidation_channel::<K, V>(
                                    query_scope_info,
                                    key_hash,
                                    &maybe_local_key,
                                );
                ...
                #[cfg(any(all(debug_assertions, feature = "devtools"), feature = "devtools-always"))]
                let elapsed_ms = chrono::Utc::now().signed_duration_since(before_time).num_milliseconds();

                let buster_if_uncached = if loading_first_time {
                    Some(maybe_buster_if_uncached.unwrap_or_else(|| ArcRwSignal::new(new_buster_id())))
                } else {
                    None
                };
                ...
                let next_directive = scope_lookup.with_cached_scope_mut::<_, _, _, _>(
                    ...
                    |scope, _| {
                        ...
                        match maybe_new_value {
                            MaybeNewValue::NewValue(new_value) => {
                                if let Some(cached) = scope.get_mut(&key_hash) {
                                    cached.set_value(
                                        ...
                                        crate::events::Event::new(crate::events::EventVariant::Fetched { elapsed_ms }),
                                    ...
                                    scope.insert_without_query_created_notif(
                                        ...
                                        Query::new(
                                            ...
                                            *self,
                                            ...
                                            maybe_local_key,
                                            new_value,
                                            buster_if_uncached.expect("loading_first_time means this is Some(). (bug)"),
                                            scope_options,
                                            None,
                                            #[cfg(any(
                                                all(debug_assertions, feature = "devtools"),
                            ...
                            MaybeNewValue::SsrStreamedValueOverride => return_cb(CachedOrFetchCbInput {
                                cached: scope
                                    .get(&key_hash)
